...
#     single_day = fcst.RZSM_anom[i:i+1,:,:,:,:]
#     single_day.where(West_conus_mask[0,:,:]==1)
#     out_ = xr.concat(fcst.RZSM_anom[i:i+1,:,:,:,:])
#     print(i)
...
def make_save_plots_all_models_seasons_leads(acc_values_to_plot,var,min_all,max_all):
    ...
    for idx,region_model in enumerate(sorted(acc_values_to_plot)):
        ...
        if idx==0:
            if model_NAM1 == 'RSMAS':
                s.set_title(f'RSMAS CCSM4 \n RZSM Continous Ranked Probability Skill Score',fontsize=25)
            elif model_NAM1 == 'GMAO':
                s.set_title(f'GMAO GEOS-5 \n ETo RZSM Continous Ranked Probability Skill Score',fontsize=25)
    ...
    plt.savefig(f'{output_season_dir}/all_season_CRPSS_skill_{var}.tif',dpi=300)
