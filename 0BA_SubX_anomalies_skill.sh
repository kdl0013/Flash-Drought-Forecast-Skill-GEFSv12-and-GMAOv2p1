...
remove_S_dimension () {
cat 1b3_remove_S_dimsension.py | sed 's|model_name|'$1'|g' | sed 's|main_dir|'${main_directory}'|g'> $data_s/TMP_1b3_remove_S_dimsension.py 

python3 TMP_1b3_remove_S_dimsension.py
}

for model in "${model_array[@]}";
do remove_S_dimension $model;done
#####################################
