...
for file in sorted(glob(f'{var}*.nc4')):
    ...
    anomaly_MEM = (open_f.ETo[:,0:3,:,:,:].mean(axis=1).to_numpy() - open_mean.ETo_mean[:,0:3,:,:,:].mean(axis=1).to_numpy())
    ...
    open_f.to_netcdf(f"{anom_dir}/ETo_anomaly_{model_NAM1}_{file.split('_')[-1]}")
    ...
    var_OUT.to_netcdf(f"{mean_ensemble_dir}/ETo_anomaly_MEM_{model_NAM1}_{file.split('_')[-1]}")
