                    ...
                    for file in dates_to_keep:
                        ...
                        if file[-14:-4] != _date:
                            ...
                            for idx,val in enumerate(b_julian_out2):
                                ...
                                if idx < 39:
                                    try:
                                        summation_ETo_modN[f'{b_julian_out2[idx+week_lead]}'].append({f'{file[-14:-4]}':np.nanmean(Et_ref_open_f.ETo.sel(lead=slice(val,b_julian_out2[idx+week_lead])).isel(S=0, model=model_NUM, X=i_X, Y=i_Y).values)})   
                                    except KeyError:
                                        pass
                ...
                def add_to_npy_file(ETo_next_dict_modN,model_NUM):
                    ...
                    #for some reason it's a list in a list, this fixes that, loop through julian day
                        ...
                        for dic_init_and_eddi_val in EDDI_final_dict:
                            ...
                            a ValueError: cannot reshape array of size 15328 into shape (2,45,27,59)'''
