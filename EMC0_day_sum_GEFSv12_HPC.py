...
For precipitation only. Daily total precip.
'''
import os
        ...
        try:
            ...
            if (len(all_files_d10) == 11) and (len(all_files_d35) == 11):
                ...
                for ensemble_number,files in enumerate(zip(all_files_d10,all_files_d35)):
                    ...
                    for i in range(35):
                        if i ==0:
                            steps[i] = open_d10[f'{var_name}'][start_:start_+7,:,:].sum(dim=['step']).values
                            start_+=8 #needed to begin the next index to keep up with proper dates
                        elif i<10:
                            steps[i] = open_d10[f'{var_name}'][start_:start_+7,:,:].sum(dim=['step']).values #eight total possible values until last time step
                            ...
                            try:
                                ...
                                s1 = (open_d35[f'{var_name}'][-1,:,:] + open_d35[f'{var_name}'][0,:,:] + \
                                    open_d35[f'{var_name}'][1,:,:] + open_d35[f'{var_name}'][2,:,:])
                                steps[i] = s1
                            ...
                            steps[i] = open_d35[f'{var_name}'][start_:start_+4,:,:].sum(dim=['step']).values
                            start_+=4
                    #Add to file
