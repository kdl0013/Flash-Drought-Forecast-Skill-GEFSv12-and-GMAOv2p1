    ...
    for i_Y in range(subx_out[f'{var}'].shape[3]):
        for i_X in range(subx_out[f'{var}'].shape[4]):
            ...
            if (HP_conus_mask.High_Plains[0,i_Y,i_X].values in np.arange(1,7)):
                ...
                def collect_all_weekly_lead_averages_from_other_files(model_mean_vals,model_anomaly_values):
                    ...
                    for file in sorted(glob(f'{home_dir}/{varname}*{_date[-5:]}.nc4')):
                    ...
                    for file in dates_to_keep:
                        ...
                        if file[day_s:day_e] != _date:
                            all_year_mod[file[day_s:day_e]]=[]
                            open_f = xr.open_dataset(file)
                            for idx,mod in enumerate(subx_out.model.values):
                                all_year_mod[file[day_s:day_e]].append({f'Mod{idx}':[]})
                                # all_year_mod[file[day_s:day_e]].append({f'Mod{idx}':[]}) #save model
                                for idx,julian_d in enumerate(subx_out.lead.values):
                                    if idx<7:
                                        if idx==0 or idx==1:
                                           all_year_mod[file[day_s:day_e]][0][f'Mod{idx}'].append({f'{julian_d}':bn.nanmean(subx_out[f'{var}'].isel(lead=idx).isel(S=0, model=0, X=i_X, Y=i_Y).values)})
                                        else:
                                            #Take the averages that we can based on idx
                                            all_mean_var_mod[mod].append({f'{julian_d}':bn.nanmean(subx_out[f'{var}'].isel(lead=slice(idx-idx+1,idx)).isel(S=0, model=0, X=i_X, Y=i_Y).values)})
                                    ...
                                    #first day cannot have a weekly mean. Just take a single value (leave np.nanmean function so array comes out properly
                                        ...
                                        try:
                                            all_year_mod[file[day_s:day_e]][f'Mod{idx}'].append(\
                                            {f'{julian_d}':np.nanmean(open_f[f'{var}'].isel(lead=slice(idx-6,idx))\
                                           .isel(S=0, model=0, X=i_X, Y=i_Y).values)})   
                    ...
                    collect_all_weekly_lead_averages_from_other_files(model_mean_vals,model_anomaly_values)
                #%%             
