...
for file in sorted(glob(f'{var}*.nc4')):
    ...
    if model_NAM1 == 'EMC':
        mean_date = f"RZSM_mean_11_models_2000-{file.split('_')[-1].split('.')[0][5:]}.nc4"
    else:
        mean_date = f"ETo_mean_2000-{file.split('_')[-1].split('.')[0][5:]}.nc4"
    ...
    open_f = open_f.rename(RZSM='RZSM_anom')
    open_f = open_f.assign_coords(L=np.arange(open_f.L.shape[0])) #now save as lead date for anomaly correlation
    open_f.to_netcdf(f"{anom_dir}/RZSM_anomaly_{model_NAM1}_{file.split('_')[-1]}")
    os.system(f"ncks -4 -L 4 {anom_dir}/RZSM_anomaly_{model_NAM1}_{file.split('_')[-1]} {anom_dir}/a_RZSM_anomaly_{model_NAM1}_{file.split('_')[-1]}")
    os.system(f"mv {anom_dir}/a_RZSM_anomaly_{model_NAM1}_{file.split('_')[-1]} {anom_dir}/RZSM_anomaly_{model_NAM1}_{file.split('_')[-1]}")
    #make new dataset for anomaly multi_ensemble mean (MEM)
    
    #Convert to an xarray object
    var_OUT = xr.Dataset(
        data_vars = dict(
            RZSM_anom_MEM = (['S','L','Y','X'],  anomaly_MEM),
        ),
        coords = dict(
            X = open_f.X.values,
            Y = open_f.Y.values,
        ...
        attrs = dict(
            Description = 'RZSM. Volumetric (m3/m3) 0-100cm depth. {model_NAM1}'),
    )              
    var_OUT.to_netcdf(f"{mean_ensemble_dir}/RZSM_anomaly_MEM_{model_NAM1}_{file.split('_')[-1]}")
    os.system(f"ncks -4 -L 4 {mean_ensemble_dir}/RZSM_anomaly_MEM_{model_NAM1}_{file.split('_')[-1]} {mean_ensemble_dir}/a_RZSM_anomaly_MEM_{model_NAM1}_{file.split('_')[-1]} ")
    os.system(f"mv {mean_ensemble_dir}/a_RZSM_anomaly_MEM_{model_NAM1}_{file.split('_')[-1]}  {mean_ensemble_dir}/RZSM_anomaly_MEM_{model_NAM1}_{file.split('_')[-1]}")
