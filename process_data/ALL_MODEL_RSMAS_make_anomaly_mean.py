...
init_date_list = return_date_list(varname)    
...
#GMAO mrso, some files are corrupted, remove them
if model_NAM1 == 'GMAO':
    for file in sorted(glob(f'{home_dir}/mrso*')):
        try:
...
def make_subX_anomaly(_date, var,HP_conus_mask,anomaly_spread, save_MME_anomaly_mean,save_MME_anomaly,init_date_list):
    ...
    if var == 'RZSM':
        ...
        subx_out = xr.open_dataset(f'{home_dir}/{varname}_{model_NAM1}_{use_date}.nc4', engine='netcdf4')
    ...
    if var == 'RZSM':
        var_OUT = xr.Dataset(
            data_vars = dict(
                RZSM_anom = (['S', 'M','L','Y','X'], out_template[list(out_template.keys())[0]].values),
            ),
            coords = dict(
