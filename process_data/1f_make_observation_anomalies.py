...
# -*- coding: utf-8 -*-
"""
Create 42 day spread for anomalies for GLEAM soil moisture and 
gridMET evaporative demand.

This script is slow because you numba cannot be used (without) really 
re-working the code and getting creative. Not worth it. Code runs fine.
...
def run_GLEAM():
    try:
        ...
        # lat=10
        # lon=10
        print('Working on creating GLEAM soil moisture anomalies. About 1 hour to completion.')
        for lat in range(open_rzsm.SMroot.shape[1]):
            for lon in range(open_rzsm.SMroot.shape[2]):
                ...
                for day in range(7,377):
                    ...
                    yearly_avg_list = list(yearly_average.values())
                    #Now choose days 42 days on either side of each year and append to a single file
                    day_val=open_rzsm.time.to_numpy()
                    for year_ in np.arange(2000,2022):
                        if pd.to_datetime(day_val).year % 4 ==0:
                            day_val = day_val+np.timedelta64(366,'D')
                        else:
                    ...
                    #the yearly values are already included in the distribution
                    #flatten the list of lists, find mean
                    mean_ = np.nanmean([x for xs in all_values for x in xs])
