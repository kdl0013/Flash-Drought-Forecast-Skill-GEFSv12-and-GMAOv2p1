...
obs_eto = xr.open_dataset(f'{dir1}/Data/MERRA2/RZSM_anomaly_MERRA.nc',chunks = {'time':1})
...
obs_eto_mean = xr.open_dataset(f'{dir1}/Data/MERRA2/RZSM_anomaly_MERRA_mean.nc')
...
if model_NAM1 == 'ESRL':
    ...
    subx_files = rename_subx_for_climpred(xr.open_mfdataset(f'{var}_anomaly_{model_NAM1}_*.nc4', concat_dim=['S'], combine='nested',chunks={'S': 1, 'L': 32})).isel(init=slice(1,-10))
    ...
    subx_files = rename_subx_for_climpred(xr.open_mfdataset(f'{var}_anomaly_{model_NAM1}_*.nc4', concat_dim=['S'], combine='nested',chunks={'S': 1, 'L': 45}).sel(S=slice('2000-01-01','2022-05-30')))
...
def select_region(cluster_num):
    ...
    for season in ['JJA','SON','DJF','MAM']:
        ...
        if cluster_num == 1:
            hindcast = HindcastEnsemble(fcst.RZSM_anom.where(West_conus_mask[0,:,:]== 1).sel(init=(fcst['init.season']==f'{season}'))).add_observations(verif.RZSM_anom.where(West_conus_mask[0,:,:]== 1))

            # bn.nanmean(all_crpss_skill)
        else:
            #Keep all seasons
            hindcast = HindcastEnsemble(fcst.RZSM_anom.where(HP_conus_mask[0,:,:]== cluster_num).sel(init=(fcst['init.season']==f'{season}'))).add_observations(verif.RZSM_anom.where(West_conus_mask[0,:,:]== cluster_num))
        
        skillACC = hindcast.verify(metric="pearson_r", comparison="e2o", dim="init", alignment="maximize")
        # bn.nanmean(skillACC.ETo_anom)
        skill_lead=skillACC.RZSM_anom[::7,:,:]
        skill_lead=skill_lead[1:,:,:].compute().to_dataset()
        
        #lead_vals = 
        for lead in range(skill_lead.lead.shape[0]):
            output_dictionary[f'Lead_{lead+1}_{season}']= skill_lead.RZSM_anom[lead,:,:].mean().values
...
def make_save_plots_all_models_seasons_leads(acc_values_to_plot,var,min_all,max_all):
    ...
    plt.savefig(f'{output_season_dir}/all_season_{metric}_skill_{var}_climpred.tif',dpi=300)
