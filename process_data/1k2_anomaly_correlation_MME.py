...
subX_dir = f'{dir1}/Data/SubX/{model_NAM1}/anomaly/MME'
os.chdir(f'{subX_dir}')
...
#X coordinate list from a file
x_coords=[235., 236., 237., 238., 239., 240., 241., 242., 243., 244., 245.,
...
y_coords = [50., 49., 48., 47., 46., 45., 44., 43., 42., 41., 40., 39., 38.,
       37., 36., 35., 34., 33., 32., 31., 30., 29., 28., 27., 26., 25.,
       24.]

lead_coords=np.array([0,1,2,3,4,5,6,3.4,3.5,3.6,4.6])

def fix_X_coord_dim(var,x_coords,y_coords,lead_coords):
    #create a new file with the lead dates (which were day of year) - but are
    #now being changed back to lead day (0-44 lead)
    file_list = sorted(glob(f'{var}_MME*.nc4'))
    for file in file_list:
        try:
            xr.open_dataset(f'{var}_MME_X_fixed_{file[-14:]}')
        except FileNotFoundError:
            open_f = xr.open_dataset(file)
            open_f.close()
            
            '''Change the lead date back to integer 0-44 instead of day of year'''
            open_f = open_f.assign_coords(X=x_coords)
            open_f = open_f.assign_coords(Y=y_coords)
            open_f=open_f.assign_coords(lead=lead_coords)
...
def all_season_mod_skill(var,cluster_num, mask_path,obs_eto_path, obs_rzsm_path):
    ...
    if var == 'ETo':
        ...
        sub_name_MME = 'ETo_MME_anom'
        sub_name = 'ETo_anom'
        obs_mean = obs_eto_mean
        obs_path = obs_eto_path
    elif var == 'RZSM':
        obs_name = 'SM_SubX_anomaly_*.nc4'
        sub_name_MME = 'RZSM_MME_anom'
        sub_name = 'RZSM_anom'
    ...
    #Don't have to load into memory because I'm immediately converting them into a np.array
    subx_files = xr.open_mfdataset(f'{var}_MME_X_fixed*.nc4', concat_dim=['S'], combine='nested')
    obs_files = xr.open_mfdataset(f'{obs_path}/{obs_name}', concat_dim = ['S'], combine = 'nested')
    ...
    if cluster_num == 1:
        summer_subx = subx_files[f'{sub_name_MME}'].sel(S=(subx_files['S.season']=='JJA')).where(West_conus_mask == cluster_num)
        fall_subx = subx_files[f'{sub_name_MME}'].sel(S=(subx_files['S.season']=='SON')).where(West_conus_mask == cluster_num)
        winter_subx= subx_files[f'{sub_name_MME}'].sel(S=(subx_files['S.season']=='DJF')).where(West_conus_mask == cluster_num)
        spring_subx= subx_files[f'{sub_name_MME}'].sel(S=(subx_files['S.season']=='MAM')).where(West_conus_mask == cluster_num)
        ...
        summer_subx = subx_files[f'{sub_name_MME}'].sel(S=(subx_files['S.season']=='JJA')).where(HP_conus_mask == cluster_num)
        fall_subx = subx_files[f'{sub_name_MME}'].sel(S=(subx_files['S.season']=='SON')).where(HP_conus_mask == cluster_num)
        winter_subx= subx_files[f'{sub_name_MME}'].sel(S=(subx_files['S.season']=='DJF')).where(HP_conus_mask == cluster_num)
        spring_subx= subx_files[f'{sub_name_MME}'].sel(S=(subx_files['S.season']=='MAM')).where(HP_conus_mask == cluster_num)
    ...
    for season in range(len(skill_subx)):
        ...
        var_OUT[:,:,:,:] = np.nan
        #Only want to save dim (lead x Y x X) [multi ensemble mean]
        var_OUT = var_OUT[0,:,:,:]
...
def setup_plot_all_leads(all_cluster_acc,cluster_num):
    ...
    for idx,k in enumerate(var_cluster.keys()):
        ...
        df_OUT=df_OUT.append({'Lead':float(split_[0][-3:]), \
...
RZSM_acc_values_to_plot_MME,max_all_rzsm, min_all_rzsm = plot_lead_week_season_model(all_vals_setup=all_vals_setup_RZSM_MME)
ETo_acc_values_to_plot_MME,max_all_eto, min_all_eto = plot_lead_week_season_model(all_vals_setup=all_vals_setup_ETo_MME)
#%%
