...
# dir1 = '/home/kdl0013/NRT_CPC_Internship' #Easley HPC
name_ = 'evap_equation'

obs_subx_eto_path = f'{dir1}/Data/MERRA2'
var='ETo'
metric='ACC'
...
all_obs_and_subx_by_model[f'Observations'] = rename_obs_for_climpred(obs_eto)
for model_NAM1 in model_list:
    print(f'Adding {model_NAM1} SubX model to dictionary all_obs_and_subx_by_model.')
    subX_dir = f'{dir1}/Data/SubX/{model_NAM1}/anomaly'
    #Open files
    try:
        subx_files = rename_subx_for_climpred(xr.open_mfdataset(f'{subX_dir}/{var}_{name_}_anomaly_{model_NAM1}_*.nc4', concat_dim=['S'], combine='nested').sel(S=slice('2000-01-01','2022-05-30')) )
    except KeyError:
        #For NRL, need to rename a file
        # all_files_keep = []
        for file in sorted(glob(f'{subX_dir}/{var}_{name_}_anomaly_{model_NAM1}_*.nc4')):
            a=xr.open_dataset(file)
            a.close()
            a['S']=np.atleast_1d(np.datetime64(pd.to_datetime(a.S.values).date[0]))
            a.to_netcdf(f'{subX_dir}/a_test.nc4')
            os.system(f'mv {subX_dir}/a_test.nc4 {file}')
            # all_files_keep.append(file)
            subx_files = rename_subx_for_climpred(xr.open_mfdataset(f'{subX_dir}/{var}_{name_}_anomaly_{model_NAM1}_*.nc4', concat_dim=['S'], combine='nested').sel(S=slice('2000-01-01','2022-05-30')) )
    ...
    #     all_obs_and_subx_by_model[f'{model_NAM1}_subx'] = rename_subx_for_climpred(subx_files)

#%%
list(all_obs_and_subx_by_model.keys())
'''Now we have all of the data saved into 1 dictionary.

The next step is to figure out how to combine all into 1 dataset, but still retain 
the information by lead time'''

#IDEA: xr.concat all together
all_models = xr.concat([all_obs_and_subx_by_model['GMAO_subx'],all_obs_and_subx_by_model['RSMAS_subx'],
           all_obs_and_subx_by_model['ECCC_subx'],all_obs_and_subx_by_model['EMC_subx'],
           all_obs_and_subx_by_model['NRL_subx'],all_obs_and_subx_by_model['ESRL_subx']],dim='init')
...
file_exists = exists(f'{output_nc_dir}/ALL_MODEL_ETo_{name_}_anomaly_climpred_ACC.pkl')
...
#Set up climpred
fcst=all_models.chunk({'init':-1})
verif=all_obs_and_subx_by_model['Observations'].chunk({'time':-1})


output_nc_dir = f'{subX_dir}/skill_assessments/' #for skill assessment .nc4 files
output_image_dir = f'{dir1}/Outputs/anomaly_correlation/{model_NAM1}'
output_season_dir = f'{dir1}/Outputs/anomaly_correlation/ALL_MODEL/seasonal_skill/'
...
#     # dates[0]
#     len(dates)
#     subx_files=subx_files.assign_coords(init=dates)
...
def select_region(cluster_num):
    ...
    for season in ['JJA','SON','DJF','MAM']:
        ...
        if cluster_num == 1:
            ...
            #Keep all seasons
            hindcast = HindcastEnsemble(fcst.ETo_anom.where(HP_conus_mask[0,:,:]== cluster_num).sel(init=(fcst['init.season']==f'{season}'))).add_observations(verif.ETo_anom.where(HP_conus_mask[0,:,:]== cluster_num))
...
for clus_num in np.arange(1,7):
    print(f'Working on Cluster {clus_num} for {metric}')
...
def make_save_plots_all_models_seasons_leads(acc_values_to_plot,var,min_all,max_all):
    ...
    plt.savefig(f'{output_season_dir}/all_model_{metric}_skill_{var}_{name_}_climpred.tif',dpi=300)
