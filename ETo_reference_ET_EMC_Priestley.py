...
# for _date in init_date_list:
    ...
    try:
        ...
        # os.system(f'rm {home_dir}/{fileOUT_name}')
...
#%%        
        ...
        if ('dlwrf' in list(locals().keys())) and ('dswrf' in list(locals().keys())) \
            and ('ulwrf' in list(locals().keys()))  \
                and ('tavg' in list(locals().keys())):
                    
            if ('uswrf' not in list(locals().keys())):
                short_rad = dswrf.dswrf.rename('short_rad')
            ...
            def date_file_info(SubX_file):
                ...
                return(a_date_out,a_start_date)
        

            julian_list, S_date = date_file_info(tavg)
            ...
            try:
                ...
                var_OUT = var_OUT.assign_coords({'S':np.atleast_1d(S_date)})
            except AttributeError:
