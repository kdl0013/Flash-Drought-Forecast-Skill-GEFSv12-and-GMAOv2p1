...
obs_eto_mean = xr.open_dataset(f'{dir1}/Data/MERRA2/ETo_anomaly_MERRA.nc')
...
def plot_lead_week_season_model(all_vals_setup,obs_files,subx_files):
    ...
    for i in all_vals_setup.keys():
        ...
        def return_data_for_plot(subset_by_cluster):
            ...
            var_OUT = np.zeros(shape = (obs_files.M.shape[0],subx_files.L.shape[0]))
...
ETo_acc_values_to_plot,max_all_eto, min_all_eto = plot_lead_week_season_model(all_vals_setup=all_vals_setup_ETo, \
                                                                              obs_files=obs_files,subx_files=subx_files)
...
make_save_plots_all_models_seasons_leads(acc_values_to_plot,var,min_all,max_all)