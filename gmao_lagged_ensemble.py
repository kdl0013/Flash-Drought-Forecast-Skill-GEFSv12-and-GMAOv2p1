...
Inputs:
    Individual SubX files for each variable. After pre-processing for the grid
    and for initialization dates.
...
model = 'GMAO'
variables = ['mrso', 'dswrf','huss','tas', 'uas', 'vas','tdps','pr','cape']
...
for var in variables:
    try:
        ...
        varFile = (varFile[f'{var}'][0,:,0,:]).to_dataset()
        ...
        # #Add 3rd dimension for number of files
        ...
        for file in sorted(glob(f'{var}*.nc')):
            ...
            try:
                ...
                def lagged_ensemble_preprocess(open_f_lead_0_ordinal, beginning_start_date_ordinal):
                    global open_f_arr
                    global var_TEST
                    for mod in prange(open_f_arr.shape[0]):
                            ...
                            for Yx in range(open_f_arr.shape[2]): 
                                ...
                                #var_TEST output 
                                    var_TEST[mod,(lead_final_ordinal - beginning_start_date_ordinal),Yx,Xx] = \
                                        (open_f_arr[mod,lead, Yx,Xx] + var_TEST[mod,(lead_final_ordinal - beginning_start_date_ordinal),Yx,Xx])/2 #append each lead to file_size_list dim
        ...
        var_OUT = xr.Dataset(
            data_vars = dict(
                mrso = (['realization','time_series_dates','Y','X'], var_TEST),
            ),
            coords = dict(
                X = var_F_unstack.X.values,
                Y = var_F_unstack.Y.values,
                time_series_dates = daily_dates,
                realization = var_F_unstack.M.values,
            ),
            attrs = dict(
