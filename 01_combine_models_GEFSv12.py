...
template_GEFS_initial = np.empty(shape=(1,11,34,27,59))
...
for var in vars_to_process:
    ...
    (fraction between wilting and saturation) GEFSv12. Daily average already computed. All ensembles and leads in one file')
            ...
            out_name1 = 'out_1.nc4'
            GEFS_out.to_netcdf(path = f"{path_out}/{out_name1}")
            #Now compress
            os.system(f'ncks -4 -L 1 {path_out}/{out_name1} {path_out}/{final_out_name}')
            #Remove the old files
            os.system(f'rm {path_out}/{out_name1}')
#%%
