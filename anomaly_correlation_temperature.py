...
#observations
obs_subx_eto_path = f'{dir1}/Data/MERRA2/tas_SubX_values/'
...
obs_name = f'tas_SubX_{model_NAM1}*.nc4'
sub_name = 'tas'
...
if model_NAM1 == "GMAO":
    ...
    for f in sorted(glob('tas_GMAO*')):
        try:
            open_f = xr.open_dataset(f)
            open_f = open_f.assign_coords(L=np.arange(open_f.L.shape[0]))
            out_file = f"temperature_{f.split('_')[1]}_{f.split('_')[2]}"
            
            open_f=open_f.where(open_f < 1e06)
            open_f.to_netcdf(out_file)
            keep_files.append(out_file)
        except ValueError:
            pass
        

    subx_files = xr.open_mfdataset(keep_files, concat_dim=['S'], combine='nested').sel(S=slice('2000-01-01','2022-05-30'))
...
def make_save_plots_all_models_seasons_leads(acc_values_to_plot,var,min_all,max_all):
    ...
    for idx,region_model in enumerate(sorted(acc_values_to_plot)):
        ...
        if idx==0:
            if model_NAM1 == 'RSMAS':
                s.set_title(f'RSMAS CCSM4 \n {var} Pearson Correlation',fontsize=25)
