...
def return_date_list(var):
    date_list = []
    for file in sorted(glob(f'{home_dir}/{var}*{evap}*.nc4')):
...
def make_subX_anomaly(_date):
    ...
    print(f'Calculating {var} {name_} mean on SubX for {_date} and saving as .nc4 in {new_dir_mean}.') 
    ...
    try:
        ...
        if var == 'RZSM':
            ...
            subx_all = xr.open_mfdataset(dates_to_keep, concat_dim=['S'], combine='nested',parallel='True').persist()
                        ...
                        for mod in all_mean_var_mod.keys():
                            ...
                            for idx,julian_d in enumerate(subx_out.L.values):
                                ...
                                try:
                                    ...
                                    if idx<7:
                                        if idx==0 or idx==1:
                                            all_mean_var_mod[mod].append({f'{julian_d}':bn.nanmean(subx_out[list(subx_out.keys())[0]].isel(L=idx).isel(S=0, M=0, X=i_X, Y=i_Y).values)})
                                        else:
                                            #Take the averages that we can based on idx
                                            all_mean_var_mod[mod].append({f'{julian_d}':bn.nanmean(subx_out[list(subx_out.keys())[0]].isel(L=slice(idx-idx+1,idx)).isel(S=0, M=0, X=i_X, Y=i_Y).values)})
                                    elif idx >= 7:
                                        #Subtract minus 6 because of indexing. First is leads 1-7, which equals 7 days
                                        all_mean_var_mod[mod].append({f'{julian_d}':bn.nanmean(subx_out[list(subx_out.keys())[0]].isel(L=slice(idx-6,idx)).isel(S=0, M=0, X=i_X, Y=i_Y).values)})
                                except IndexError:
                                    pass                    
        ...
        #TODO: For some reason, you need to multiply ESRL by negative 1
        var_OUT = xr.Dataset(
            data_vars = dict(
                ETo_mean = (['S', 'M','L','Y','X'], out_template[list(out_template.keys())[0]].values * -1),
            ),
            coords = dict(
