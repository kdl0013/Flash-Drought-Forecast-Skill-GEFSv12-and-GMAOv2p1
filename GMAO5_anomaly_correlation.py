...
#observations
obs_subx_eto_path = f'{dir1}/Data/MERRA2/ETo_{model_NAM1}_SubX_values/'
...
def setup_plot_all_leads(all_cluster_acc,cluster_num):
    ...
    for idx,k in enumerate(var_cluster.keys()):
        ...
        df_OUT=df_OUT.append({'Lead':float(split_[1][-3:]), \
...
if __name__ == '__main__':
    p = Pool(6)
    global all_vals_setup_ETo
    all_vals_setup_ETo = {}
    p.map(setup_plot_all_leads,np.array(np.arange(1,7)))
