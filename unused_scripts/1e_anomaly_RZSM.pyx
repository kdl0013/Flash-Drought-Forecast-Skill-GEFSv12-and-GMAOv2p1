            ...
            if (np.count_nonzero(np.isnan(smerge_file.RZSM[0,i_Y,i_X].values)) !=1):
                ...
                def dict1_subx2():
                    ...
                    def limit_file_list(str _date):
                        ...
                        for file in sorted(glob(f'{rzsm_dir}/SM*.nc4')):
                    ...
                    for file in dates_to_keep:
                        ...
                        if file[-14:-4] != _date:
                            ...
                            for idx,val in enumerate(b_julian_out2):
                                ...
                                if idx < 39:
                                    try:
                                        summation_ETo_modN[f'{b_julian_out2[idx+week_lead]}'].append({f'{file[-14:-4]}':np.nanmean(Et_ref_open_f.SM_SubX_m3_m3_value.sel(lead=slice(val,b_julian_out2[idx+week_lead])).isel(S=0, model=model_NUM, X=i_X, Y=i_Y).values)})   
                                    except KeyError:
                                        pass
                ...
                def add_to_npy_file(dict RZSM_next_dict_modN,int model_NUM):
                    ...
                    #for some reason it's a list in a list, this fixes that, loop through julian day
                        ...
                        for dic_init_and_eddi_val in EDDI_final_dict:
                            ...
                            # fileOut = f'RZSM_anomaly_{init_day}.npy'
                            ...
                            # lead_values = np.load(f'RZSM_anomaly_{init_day}_julian_lead.npy',allow_pickle=True)
                            
                            # #Sometimes we found some files with indexes that really don't matter because they are on the
                            # #outskirts of the 3 month window.
                            # try:
