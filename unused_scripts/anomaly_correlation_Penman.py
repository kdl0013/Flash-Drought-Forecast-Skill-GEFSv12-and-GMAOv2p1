...
def all_season_mod_skill(cluster_num,obs_files,subx_files):
    ...
    print(f'Working on cluster {cluster_num} out of 6 for ETo {name_}.')
        ...
        # subx_files=subx_files.sel(S=~subx_files.get_index('S').duplicated()) #remove duplicate that was causing an issue
        # subx_files=subx_files.sel(S=obs_files.S.values)
        # #need to make the S a datetime object
        # out_date = []
        # for i in range(subx_files.S.values):
        #     out_date.append(subx_files.S.values[i].date())
            
        # [i[0].date() for i in subx_files.S.values]
        
        # aa=subx_files.S.values
        # aa[0].date()
        # datetime.datetime.strptime(pd.to_datetime(aa[0]), '%Y-%m-%d')
        # datetime.date(pd.to_datetime(aa[0]))
        # subx_files.assign_coords(S=subx_files.S.values.date())
        # pd.to_datetime(subx_files.S.values)
...
#     all_cluster_acc_RZSM[f'Cluster {clus_num}'] = all_season_mod_skill(var='RZSM',cluster_num=clus_num, mask_path=mask_path,obs_eto_path=obs_subx_eto_path)
    ...
    for i in all_vals_setup.keys():
        ...
        def return_data_for_plot(subset_by_cluster):
            ...
            if model_NAM1 == 'ESRL':
                ...
                #We are 1 day short shy of a 5th week
                Cols = ['1', '2', '3', '4']
            else:
                Cols = ['1', '2', '3', '4', '5', '6']
...
#%%
def make_save_plots_all_models_seasons_leads(acc_values_to_plot,var,min_all,max_all,name_):
...
make_save_plots_all_models_seasons_leads(acc_values_to_plot=ETo_acc_values_to_plot,var=var,min_all=min_all,max_all=max_all,name_=name_)
