...
# -*- coding: utf-8 -*-
"""
Goal is to look at each SubX file for soil moisture, EDDI, and reference ET and look at the
...
Also this script converts SubX to m3/m3 based on the soil profile depth.
...
SM_SubX_anomaly_out_dir = f'{SM_SubX_out_dir}/SM_SubX_anomalies'
#EDDI
...
os.system(f'mkdir -p {SM_SubX_out_dir}/SM_SubX_anomalies')
...
#%%
def EDDI_SubX_creation(_date) -> float:    
    # for _date in date_list:
    try:
        xr.open_dataset(f'{eddi_subX_dir}/EDDI_SubX_{mod}_{_date}.nc')
        ...
        print(f'Working on initialized day {_date} to find EDDI values from SubX models, leads, & coordinates and saving data into {eddi_subX_dir}.')
        ...
        sub_file_arr = sub_file.EDDI[0,:,:,:,:].values
        
        
        '''Now that we have the subx EDDI file open, now we need to open up the 
        EDDI time series (all a single file). Select the same dates as SubX for 
        ...
        for model in range(sub_file.EDDI.shape[1]):
            for i_lead in range(sub_file.EDDI.shape[2]):
                date_val = pd.to_datetime(sub_file.S.values[0]) + dt.timedelta(days=i_lead)
            
                for i_Y in range(sub_file.EDDI.shape[3]):
                    for i_X in range(sub_file.EDDI.shape[4]):
        ...
        var_OUT.to_netcdf(path = f'{eddi_subX_dir}/EDDI_SubX_{mod}_{_date}.nc', mode ='w')
...
#%% ETo gridMET
#TODO: On next model (e.g., CFSv2), update this code below to include the model name
#like the above function EDDI_SubX_creation
            ...
            for i_lead in range(sub_file.ETo.shape[2]):
                ...
                for i_Y in range(sub_file.ETo.shape[3]):
                    for i_X in range(sub_file.ETo.shape[4]):
                        ...
                        gridMET_out[model, i_lead, i_Y, i_X] = \
                            gridMET_file.ETo_gridmet.sel(day = date_val).isel(lon = i_X, lat = i_Y).values
        
        #Convert to an xarray object
        var_OUT = xr.Dataset(
            data_vars = dict(
                ETo_SubX_value = (['model','lead','Y','X'], gridMET_out[:,:,:,:]),
            ),
            coords = dict(
                X = sub_file.X.values,
                Y = sub_file.Y.values,
...
def convert_SubX_to_m3_m3(_date):
    ...
    of soil column in meters (which we convert to mm). Source from GMAO researchers'''
...
def SM_SMERGE_SubX_creation(_date):    
    try:
        ...
        #Remove dimension S[1] because it has not data (it's a GMAO GEOS specific issue)
        sub_file_arr = sub_file.mrso[0,:,:,:,:].values
            ...
            for i_lead in range(sub_file.mrso.shape[2]):
                ...
                for i_Y in range(sub_file.mrso.shape[3]):
                    for i_X in range(sub_file.mrso.shape[4]):
                        ...
                        smerge_out[model, i_lead, i_Y, i_X] = \
                            SMERGE_file.RZSM.sel(time = date_val).isel(lon = i_X, lat = i_Y).values
        
        #Convert to an xarray object
        var_OUT = xr.Dataset(
            data_vars = dict(
                SMERGE_SubX_value = (['model','lead','Y','X'], smerge_out[:,:,:,:]),
            ),
            coords = dict(
                X = sub_file.X.values,
                Y = sub_file.Y.values,
...
#%%
def SM_SMERGE_ANOMALY_SubX_creation(_date):    
    try:
        xr.open_dataset(f'{SM_SubX_anomaly_out_dir}/SM_SubX_anomaly_{_date}.nc')
        print(f'Already completed date {_date}. Saved in {SM_SubX_out_dir}.')
        
    except FileNotFoundError:
            
        print(f'Working on initialized day {_date} to find SMERGE values from SubX models, leads, & coordinates and saving data into {SM_SubX_out_dir}.')
        sub_file = xr.open_dataset(f'RZSM_anomaly_{_date}.nc4')
        ...
        var_OUT = xr.Dataset(
            data_vars = dict(
                SMERGE_SubX_anomaly = (['model','lead','Y','X'], smerge_out[:,:,:,:]),
            ),
            coords = dict(
                X = sub_file.X.values,
                Y = sub_file.Y.values,
            ...
            attrs = dict(
                Description = 'SMERGE SM anomaly (m3/m3) values on the exact same date and grid \
                cell as SubX data'),
        )                    
            
        #Save as a netcdf for later processing
        var_OUT.to_netcdf(path = f'{SM_SubX_anomaly_out_dir}/SM_SubX_anomaly_{_date}.nc', mode ='w')
        #To find the correct date
...
if __name__ == '__main__':
    ...
    print('Working on RZSM anomaly')
    p.map(SM_SMERGE_ANOMALY_SubX_creation, date_list)




#%%
#--NOT NEEDED, but this code can be re-factored for future use
...
#         x,y,mean_x,mean_y = plot_setup(date_list)
