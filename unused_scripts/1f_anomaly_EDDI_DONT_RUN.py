...
init_date_list = return_date_list(var = 'EDDI')    
...
def EDDI_anomaly(start_,end_,model_NUM,init_date_list,_date):
    ...
    subx,file_timestamp_list,file_julian_list,file_datetime,variable = date_file_info(_date=_date,variable='EDDI')
    ...
    for i_Y in range(subx2[f'{variable}'].shape[3]):
        #for i_X in range(1):
         for i_X in range(subx2[f'{variable}'].shape[4]):
            ...
            if (np.count_nonzero(np.isnan(eddi_file.EDDI[0,0,10,i_Y,i_X].values)) !=1):
                ...
                def dict1_subx2():
                    ...
                    for idx,julian_d in enumerate(file_julian_list):
                        ...
                        '''EDDI calculation initially required a 7-day ETo sum, which means we lost 7 days worth of data
                        -now that we are geting the 7-day average of EDDI, we have to start with the 7th day 
                        which means that days 7-14 are required to give an EDDI anomaly estimate'''
                    ...
                    def limit_file_list(_date):
                        ...
                        for file in sorted(glob(f'{home_dir}/EDDI*.nc4')):
                ...
                '''
                def compute_anomaly(SM_7_day_average_modN):
                    out_eddi_dictionary = {}
                    ...
                    for idx,julian_date in enumerate(SM_7_day_average_modN):
                        ...
                        mean_value = np.nanmean(np.array(mean_value))
                        #calculate anomaly
                        anomaly_val = []
                        for date_init in range(len(subset_by_date)):
                            anomaly_val.append((list(subset_by_date[date_init].values())[0]-mean_value))
                ...
                RZSM_dict_modN = compute_anomaly(SM_7_day_average_modN)
                ...
                def improve_RZSM_dictionary(SM_7_day_average_modN, RZSM_dict_modN):
                ...
                def add_to_npy_file(RZSM_next_dict_modN,model_NUM):
                    ...
                    #for some reason it's a list in a list, this fixes that, loop through julian day
                        ...
                        for dic_init_and_eddi_val in EDDI_final_dict:
                            ...
                            fileOut = f'EDDI_anomaly_{init_day}.npy'
                            ...
                            lead_values = np.load(f'EDDI_anomaly_{init_day}_julian_lead.npy',allow_pickle=True)
                ...
                add_to_npy_file(RZSM_next_dict_modN,model_NUM)
                
                #release memory (didn't work)
                # del EDDI_next_dict_modN, EDDI_dict_modN, ETo_7_day_modN
                
    print(f'Completed date {_date} and saved into {home_dir}/EDDI_anomaly_mod{model_NUM}.')
    #save the dates that were completed to not re-run
    if model_NUM == 3:
        os.system(f'echo Completed {_date} >> {script_dir}/EDDI_completed_anomaly_npy_{model_NAM1}.txt')
#%%
...
completed_dates = np.loadtxt(f'{script_dir}/EDDI_completed_anomaly_npy_{model_NAM1}.txt',dtype='str')
try:
...
new_directory = f'{home_dir}/EDDI_anomaly_mod{model_NUM}/already_completed'
os.system(f'mkdir -p {new_directory}')
count=0
for _date in init_date_list[start_:end_]:    
    if _date[5:] not in subset_completed_dates:
        EDDI_anomaly(start_, end_, model_NUM, init_date_list, _date)
        count+=1
        #save
        os.system('sleep 15') #sometimes saving seems to not fully capture 1 file
        os.system(f'cp {home_dir}/EDDI_anomaly_mod{model_NUM}/*.npy {new_directory}/')
        if count == 2:
            exit()
