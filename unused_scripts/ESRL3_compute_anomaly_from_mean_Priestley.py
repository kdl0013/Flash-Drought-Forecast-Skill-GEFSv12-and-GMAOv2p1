...
for file in sorted(glob(f'{var}*.nc4')):
    ...
    mean_date = f"ETo_{evap}_mean_2000-{file.split('_')[-1].split('.')[0][5:]}.nc4"
    open_mean = xr.open_dataset(f'{mean_dir}/{mean_date}')
    #for some reason, you have to convert the datasets. Can't just subtract
    #some models don't have data    
    try:
        anomaly = (open_f.ETo[:,:,:,:,:].to_numpy() - open_mean.ETo_mean[:,:,:,:,:].to_numpy())
    except ValueError:
        try:
            anomaly = (open_f.ETo[:,0:open_mean.M.shape[0],:,:,:].to_numpy() - open_mean.ETo_mean[:,:,:,:,:].to_numpy())
        except ValueError:
            anomaly = (open_f.ETo[:,:,:,:,:].to_numpy() - open_mean.ETo_mean[:,0:open_f.M.shape[0],:,:,:].to_numpy())

    anomaly_MEM = (open_f.ETo[:,0:open_f.M.shape[0],:,:,:].mean(axis=1).to_numpy() - open_mean.ETo_mean[:,0:open_f.M.shape[0],:,:,:].mean(axis=1).to_numpy())
    
    open_f.ETo[:,0:anomaly.shape[1],:,:,:] = anomaly
    ...
    open_f.to_netcdf(f"{anom_dir}/ETo_anomaly_{evap}_{model_NAM1}_{file.split('_')[-1]}")
    ...
    var_OUT.to_netcdf(f"{mean_ensemble_dir}/ETo_anomaly_{evap}_MEM_{model_NAM1}_{file.split('_')[-1]}")
