...
# for _date in init_date_list:
    ...
    try:
        ...
        if (('dswrf' in list(locals().keys())) and ('tasmin' in list(locals().keys())) \
            ...
            windSpeed.to_dataset(name='windspeed').to_netcdf(f'windspeed_{mod}_{_date}')
                ...
                for i_Y in range(tasmax.Y.shape[0]):
                    for i_X in range(tasmax.X.shape[0]):
                        ...
                        if conus_mask.CONUS_mask[0,i_Y,i_X].values == 1:
                            ...
                            output_f.tasmin[0,:, i_lead, i_Y, i_X] = \
                                (refet.Daily(tmin = tasmin.tasmin[0,:, i_lead, i_Y, i_X].values, \
                                            tmax = tasmax.tasmax[0,:, i_lead, i_Y, i_X].values, \
                                            ea = ea[0,:, i_lead, i_Y, i_X].values, \
                                            rs = srad.srad[0,:, i_lead, i_Y, i_X].values, \
                                            uz = windSpeed[0,:, i_lead, i_Y, i_X].values, \
    ...
    try:
        ...
        if (('dswrf' in list(locals().keys())) and ('tasmin' in list(locals().keys())) \
            ...
            for model_n in range(tasmax.M.shape[0]):
                for Y in range(tasmax.Y.shape[0]):
                    for X in range(tasmax.X.shape[0]):
                        mixing_ratio[0,model_n,:,Y,X] = metpy.calc.mixing_ratio_from_specific_humidity(specific_humidity=huss.spfh[0,model_n,:,Y,X])
            ...
            for model_n in range(tasmax.M.shape[0]):
                for Y in range(tasmax.Y.shape[0]):
                    for X in range(tasmax.X.shape[0]):
                        ...
                        ea[0,model_n,:,Y,X] = metpy.calc.vapor_pressure(pressure=atm_pressure.data[0,Y,X], mixing_ratio=mixing_ratio[0,model_n,:,Y,X]*units(''))
...
# for _date in init_date_list:
    ...
    try:
        ...
        try:
            '''Returns shorwave radiation in MJ/m2 from W/m2'''
            rad = xr.open_dataset(f'rad_{mod}_{_date}.nc4')
            # dswrf.dswrf.values
            #86000 seconds in 1 day, 1000000J in 1 MJ https://www.anycodings.com/1questions/2366349/solar-energy-conversion-wm2-to-mjm2
            mult_ = 0.0864
            uswrf = np.multiply(rad,mult_) #convert to MJ/m2
            # dswrf.dswrf.values
        except IndexError:
            pass
        except ValueError:
        ...
        if ('tasmin' in list(locals().keys())) and ('rad' in list(locals().keys())) \
            ...
            #TODO: metpy.calc.dewpoint_from_specific_humidity(pressure, temperature, specific_humidity)
            #get dewpoint, save as a dataset.
            ...
            for model_n in range(tasmax.M.shape[0]):
                for Y in range(tasmax.Y.shape[0]):
                    for X in range(tasmax.X.shape[0]):
                        ...
                        mixing_ratio[0,model_n,:,Y,X] = metpy.calc.mixing_ratio_from_specific_humidity(specific_humidity=huss.huss[0,model_n,:,Y,X])
            ...
            var_OUT=var_OUT.where(var_OUT.ETo<=0)
...
#%% ESRL
'''Compute Reference ET for SubX data'''
def multiProcess_Refet_SubX_ESRL(_date):
    # _date=init_date_list[0]
# for _date in init_date_list:
    # print(_date)
    fileOUT_name=f'ETo_Penman_{mod}_{_date}.nc4'
    try:
        ...
        try:
            ...
            #86000 seconds in 1 day, 1000000J in 1 MJ https://www.anycodings.com/1questions/2366349/solar-energy-conversion-wm2-to-mjm2
            mult_ = 0.0864
            rad = np.multiply(rad,mult_) #convert to MJ/m2
            # dswrf.dswrf.values
        except IndexError:
            pass
        except ValueError:
