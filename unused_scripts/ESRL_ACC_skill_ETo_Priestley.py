...
if model_NAM1 == 'ESRL':
    ...
    subx_files = rename_subx_for_climpred(xr.open_mfdataset(f'{var}_{name_}_anomaly_{model_NAM1}_*.nc4', concat_dim=['S'], combine='nested',chunks={'S': 1, 'L': 45}))
    # subx_files = subx_files.sel(S=obs_files.S.values)

    dates_list = []
    
    for idx,val in enumerate(subx_files.init.values):
        # print(i)
        dates_list.append(datetime.date(val))
    # np.array([parser.parse(i).strftime('%Y-%m-%d') for i in subx_files.init.values])
    # date = [x.split()[0] for x in dates_list]
    # df = pd.DataFrame({'dates':dates_list})
    # df['dates'] = pd.to_datetime(df['dates'])
    # df['dates'] = df['dates'].dt.floor('d')
