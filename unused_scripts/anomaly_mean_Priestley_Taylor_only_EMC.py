...
def make_subX_anomaly(_date):
    ...
    if var == 'RZSM':
        ...
        subx_all = xr.open_mfdataset(dates_to_keep, concat_dim=['S'], combine='nested',parallel='True').isel(M=slice(1,11))
        ...
        '''Load them seperately to save on memory, otherwise it makes 1000 files with 0s in becuase 
        there are different models between different years after 2020'''
        big_model_subx = xr.open_mfdataset(large_list, concat_dim=['S'], combine='nested',parallel='True')
        small_model_subx = xr.open_mfdataset(small_list, concat_dim=['S'], combine='nested',parallel='True')
        
        # big_model_subx = xr.open_mfdataset(large_list, concat_dim=['S'], combine='nested',parallel='True').persist()
        # small_model_subx = xr.open_mfdataset(small_list, concat_dim=['S'], combine='nested',parallel='True').persist()
    ...
    out_template = xr.zeros_like(big_model_subx)
    ...
    for i_Y in range(subx_out.Y.shape[0]):
        for i_X in range(subx_out.X.shape[0]):
            ...
            if (HP_conus_mask.High_Plains[0,i_Y,i_X].values in np.arange(1,7)):
                ...
                def find_anomaly_with_weekly_mean(all_mean_var_mod):
                    ...
                    for k, julian_d in enumerate((all_mean_var_mod)):
                        ...
                        anomaly_multiprocess[mod].append({str(julian_d):mean_value})
                        return(anomaly_multiprocess)
