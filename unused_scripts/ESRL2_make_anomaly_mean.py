                    ...
                    for mod in all_mean_var_mod.keys():
                        ...
                        for idx,julian_d in enumerate(subx_out.lead.values):
                            ...
                            try:
                                ...
                                if idx<7:
                                    if idx==0 or idx==1:
                                        all_mean_var_mod[mod].append({f'{julian_d}':bn.nanmean(subx_out[list(subx_out.keys())[0]].isel(lead=idx).isel(S=0, model=0, X=i_X, Y=i_Y).values)})
                                    else:
                                        #Take the averages that we can based on idx
                                        all_mean_var_mod[mod].append({f'{julian_d}':bn.nanmean(subx_out[list(subx_out.keys())[0]].isel(lead=slice(idx-idx+1,idx)).isel(S=0, model=0, X=i_X, Y=i_Y).values)})
                                elif idx >= 7:
                                    #Subtract minus 6 because of indexing. First is leads 1-7, which equals 7 days
                                    all_mean_var_mod[mod].append({f'{julian_d}':bn.nanmean(subx_out[list(subx_out.keys())[0]].isel(lead=slice(idx-6,idx)).isel(S=0, model=0, X=i_X, Y=i_Y).values)})
                            except IndexError:
                                pass                    
