...
def EDDI_function(start_,end_,model_NUM,init_date_list,_date):
        ...
        #for i_X in range(1):
         for i_X in range(subx2.ETo.shape[4]):
            ...
            if (np.count_nonzero(np.isnan(smerge_file.RZSM[0,i_Y,i_X].values)) !=1) or ((i_X == 38 and i_Y == 6) or (i_X ==38 and i_Y ==7)):
                ...
                '''
                def compute_EDDI_on_ETo_7_day_sum_dict(ETo_7_day_modN):
                    out_eddi_dictionary = {}
                ...
                def improve_EDDI_dictionary(ETo_7_day_modN, EDDI_dict_modN):
                ...
                def add_to_npy_file(EDDI_next_dict_modN,model_NUM):
...
for _date in init_date_list[start_:end_]:    
    if _date[5:] not in subset_completed_dates:
        EDDI_function(start_, end_, model_NUM, init_date_list, _date)
        count+=1
        #save
