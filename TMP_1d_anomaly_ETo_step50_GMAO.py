                    ...
                    def find_anomaly(summation_ETo_modN,mod_number):
                        ...
                        for k, julian_d in enumerate(summation_ETo_modN):
                            ...
                            if int(julian_d) <=42:
                                ...
                                sub_small2 = subx_all[f'{var_name}'][:,mod_number,:,i_Y,i_X].sel(lead=slice(1,int(julian_d)))
                                sub_out = xr.concat([sub_small1,sub_small2],dim='lead')
                            elif julian_d >= (366-max_days):
                                add_ = 366-julian_d
                                diff_ = add_ - max_days
                                sub_small1 = subx_all[f'{var_name}'][:,mod_number,:,i_Y,i_X].sel(lead=slice(1,add_))
                                sub_small2 = subx_all[f'{var_name}'][:,mod_number,:,i_Y,i_X].sel(lead=slice(julian_d+diff_,julian_d))
                                sub_out = xr.concat([sub_small1,sub_small2],dim='S')
                            else:
                                sub_out = subx_all[f'{var_name}'][:,mod_number,:,i_Y,i_X].sel(lead=slice(julian_d-max_days,julian_d+max_days)) 
