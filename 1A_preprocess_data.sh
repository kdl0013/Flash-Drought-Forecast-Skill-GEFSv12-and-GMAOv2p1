...
cat $data_s/1a_GMAO_EDDI_ET0_single_file.py | sed 's|main_dir|'${main_directory}'|g' | \
    sed 's|procs|'${processors}'|g' > $data_s/1a_TMP_GMAO_EDDI_ET0_single_file.py

echo Starting on daily reference ET calculation

python3 $data_s/1a_TMP_GMAO_EDDI_ET0_single_file.py
rm $data_s/1a_TMP_GMAO_EDDI_ET0_single_file.py
