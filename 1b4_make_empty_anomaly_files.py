...
new_acc_dir = f'{home_dir}/mean_for_ACC'
...
def make_empty_nc_files(init_date_list,T_FILE, var):
    ...
    for _date in init_date_list:
        ...
        try:
            ...
            var_final.to_netcdf(path = f'{home_dir}/{filename}_{_date}.nc4')
...
def make_empty_mean_nc_files(init_date_list,T_FILE, var):
    ...
    for _date in init_date_list[0:73]:
        ...
        try:
            ...
            if var == 'RZSM':
                var_final = xr.Dataset(
                    data_vars = dict(
                        RZSM_mean = (['S','model','lead','Y','X'], template.ETo.values),
                    ),
                    coords = dict(
                        X = template.X.values,
                        Y = template.Y.values,
                ...
                var_final = xr.Dataset(
                    data_vars = dict(
                        EDDI_mean = (['S','model','lead','Y','X'], template.ETo.values),
                    ),
                    coords = dict(
                        X = template.X.values,
                        Y = template.Y.values,
                ...
                var_final = xr.Dataset(
                    data_vars = dict(
                        ETo_mean = (['S','model','lead','Y','X'], template.ETo.values),
                    ),
                    coords = dict(
                        X = template.X.values,
                        Y = template.Y.values,
...
make_empty_mean_nc_files(init_date_list,T_FILE, var)
...
for name in [new_eddi,new_eto_anom,new_rzsm,new_eto_mean,new_rzsm_mean]:
    try:
