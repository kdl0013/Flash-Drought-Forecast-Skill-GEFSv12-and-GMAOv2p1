...
date_list = sorted(glob(f'RZSM_*.nc4'))
...
# _date='1999-12-06'
#%% ETo gridMET
def anomaly_RZSM_SubX_creation(_date) -> float:    
    file_name = f'RZSM_SubX_anomaly_{model_}_{_date}.nc4'
    ...
    try:
        ...
        sub_file = xr.open_dataset(f'{subX_dir}/RZSM_{model_}_{_date}.nc4')
        out_file = xr.zeros_like(sub_file)
    
        #Open gridMET and subset to the correct dates as SubX for full time series
        obs_file = xr.open_dataset(f'{obs_dir}/RZSM_anomaly_MERRA.nc').astype('float64')
        # obs_file.close()
        ...
        for i_lead in range(sub_file[name(sub_file)].shape[2]):
            ...
            if np.count_nonzero(obs_file[name(obs_file)].sel(time = date_val).values == 0) == 1593:
                ...
                out_file[name(out_file)][0,:, i_lead, :, :] = \
                np.nanmean( obs_file[name(obs_file)].sel(time = date_val1).values + \
                    obs_file[name(obs_file)].sel(time = date_val2).values)
            else:
                out_file[name(out_file)][0,:, i_lead, :, :] = \
                    obs_file[name(obs_file)].sel(time = date_val).values
        ...
        var_OUT = xr.Dataset(
            data_vars = dict(
                RZSM_anom = (['S','L','Y','X'], out_file[name(out_file)].values),
            ),
            coords = dict(
            ...
            attrs = dict(
                Description = f'MERRA2 reference RZSM anomaly values on the exact same date and grid \
                cell as {model_} SubX data'),
        )                    
        

        #Save as a netcdf for later processing
        var_OUT.to_netcdf(path = f'{output_RZSM_dir}/{file_name}', mode ='w')
        ...
        print(f'Completed RZSM_SubX_{_date}')  
...
if __name__ == '__main__':
    p = Pool(n_processors)
    p.map(anomaly_RZSM_SubX_creation,date_list)
