...
for file in sorted(glob(f'{var}*.nc4')):
    try:
        xr.open_dataset(f"{mean_ensemble_dir}/ETo_{name_}_anomaly_MEM_{model_NAM1}_{file.split('_')[-1]}")
        ...
        var_OUT = xr.Dataset(
            ...
            attrs = dict(
                Description = 'Reference crop evapotranspiration (mm/day). {name_} formula'),
        )              
