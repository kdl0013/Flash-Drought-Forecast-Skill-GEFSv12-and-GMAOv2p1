...
"""
Notes:
    ESRL, it appears that you need to multiply solar radiation by PET by negative 1 (-1) to actually 
    get the correct

    ESRL - W/m2 -- net surface radiation. Convert to MJ/m2/day  np.multiply(srad,0.0864) #convert to MJ/m2
    ...
    def priestley_taylor(tavg1,srad1,elevation1,ptC1):
        ...
        PET = np.multiply(-1,PET)  #THIS SEEMS SPECIFIC TO ESRL.
            ...
            for i_mod in range(tavg[list(tavg.keys())[0]].M.shape[0]):
                for i_lead in range(tavg[list(tavg.keys())[0]].L.shape[0]):
                    try:
                        ...
                        tavg1=tavg[list(tavg.keys())[0]].isel(time=i_lead,M=i_mod).values[0] #5 files have different names
